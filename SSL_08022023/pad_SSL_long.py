...
for name, values in split_ids.items():
    splitted[name] = RelativePositioningDataset(
        [d for d in windows_dataset.datasets
        if d.description['subject'].split("@)[0] in values])
...
model = ContrastiveNet(emb, emb_size, dropout=0.3).to(device)
model.emb.children

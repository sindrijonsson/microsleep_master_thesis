function data = create_lstm_data2(features, params)
...
numWins = length(features) - (winSizeSamples-1);
winMask = repmat(1:45,numWins,1);
winStep = 1 * (0:numWins-1)';
windows = winMask+winStep;

middleSeqX = cellfun(@(x) mat2cell(features(:,x), numFeatures, winSizeSamples), ...
                        num2cell(windows,2));

startPadIdx = generate_padding_indexes(winSizeSamples);
endPadIdx = (size(features,2)+1)-flip(startPadIdx,2); 
...
% Concat data
data = [startPadSeqX; middleSeqX; endPadSeqX];
